...
export default function CreateDoacaoMaterial() {
    ...
    async function onSubmit(e) {
        ...
        try {
            ...
            window.alert("Doação material cadastrada com sucesso!");
            navigate("/doacoes", { state: { filter: "Material" } });
        } catch (error) {
    ...
    return (
        <div className="container-fluid py-2">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h3 style={{ color: primaryColor, fontWeight: 'bold' }}>Cadastrar Doação Material</h3>
                    <p className="text-muted mb-0">Registre uma nova contribuição de itens para a ONG</p>
                </div>
                <button 
                    type="button"
                    onClick={() => navigate(-1)} 
            ...
            <form onSubmit={onSubmit}>
                <div className="card border-0 shadow-sm p-4 mb-4" style={{ borderRadius: '8px' }}>
                    ...
                    <div className="row">
                        <div className="form-group col-md-6 mb-3">
                            <label style={labelStyle}>Tipo de Doação</label>
                            <select className="form-select px-3 py-2" style={inputStyle} value={form.tipo_doacao} onChange={(e) => updateForm({ tipo_doacao: e.target.value })} required>
                    ...
                    <div className="row">
                        <div className="form-group col-md-12 mb-3">
                            <label style={labelStyle}>Entrega / Coleta</label>
                            <input type="text" className="form-control px-3 py-2" style={inputStyle} value={form.forma_entrega} onChange={(e) => updateForm({ forma_entrega: e.target.value })} placeholder="Ex: Deixou na sede / Buscar no local" />
                        </div>
                    </div>
                </div>
